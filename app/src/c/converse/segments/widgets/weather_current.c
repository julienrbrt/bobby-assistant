...
WeatherCurrentWidget* weather_current_widget_create(GRect rect, ConversationEntry* entry) {
  ...
  int layer_height = (fonts->title_font_cap * 3 + 1) / 2 + (fonts->content_font_cap * 3 + 1) / 2 + (fonts->small_font_cap * 15 + 2) / 4;
...
static void prv_layer_update(Layer *layer, GContext *ctx) {
  ...
  graphics_draw_text(ctx, data->temp_string, fonts->content_font, GRect(5, top, bounds.size.w, (fonts->content_font_cap * 7 + 2) / 4), GTextOverflowModeTrailingEllipsis, GTextAlignmentLeft, NULL);
  top += (fonts->content_font_cap * 3 + 1) / 2;
  graphics_draw_text(ctx, data->feels_like_string, fonts->small_font, GRect(5, top, bounds.size.w - 10, (fonts->small_font_cap * 7 + 2) / 4), GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);
  top += (fonts->small_font_cap * 3 + 1) / 2;
  graphics_draw_text(ctx, widget->summary, fonts->small_font, GRect(5, top, bounds.size.w - 10, (fonts->small_font_cap * 7 + 2) / 4), GTextOverflowModeFill, GTextAlignmentLeft, NULL);
